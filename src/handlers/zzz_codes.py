...
class ZZZCodeScanner(commands.Cog):
    ...
    async def poll(self):
        ...
        logger.info(f"New code is available for ZZZ: {new_codes}")
        ...
        logger.info(f"ZZZ code check completed")

    async def send_notification(self, codes: Iterable[str]):
        embed = discord.Embed(
            title="New codes available",
            description="\n".join(
                f"[{code}](https://zenless.hoyoverse.com/redemption?code={code})"
                for code in codes
            )
        )
    ...
    async def redeem(self, codes: Iterable[str]):
        ...
        for code in codes:
            ...
            for account in accounts:
                if not account.settings[Preferences.ZZZ_AUTO_REDEEM]:
                    continue
                logger.info(f"Redeeming code {code} for account {account.mihoyo_id}")
                queue.append(asyncio.create_task(account.client.redeem_code(code=code,game=genshin.Game.ZZZ)))

...
class HSRCodeScanner(commands.Cog):
    ...
    async def send_notification(self, codes: Iterable[str]):
        embed = discord.Embed(
            ...
            description="\n".join(
                f"[{code}](https://hsr.hoyoverse.com/gift?code={code})"
                for code in codes
            )
        )
    ...
    async def redeem(self, codes: Iterable[str]):
        ...
        for code in codes:
            ...
            for account in accounts:
                if not account.settings[Preferences.HSR_AUTO_REDEEM]:
                    continue
                logger.info(f"Redeeming code {code} for account {account.mihoyo_id}")
                queue.append(asyncio.create_task(account.client.redeem_code(code=code,game=genshin.Game.STARRAIL)))

...
class GenshinCodeScanner(commands.Cog):
    ...
    async def poll(self):
        ...
        if codes.issubset(existing_codes):
            logger.info(f"No new codes found for Genshin, code check completed")
            return

        new_codes = codes - existing_codes
        logger.info(f"New code is available for Genshin: {new_codes}")
        ...
        logger.info(f"Genshin code check completed")
